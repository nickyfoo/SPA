    ...
    for (SuchThatClause *relationship : *relationships_) {
        ...
        switch (relationship_type) {
            ...
            case RelRef::FollowsT: {
                FollowsParentsHandler *follows_parents_handler =
                        FollowsParentsHandler::get_instance();
                follows_parents_handler->set_function_pointers(&Statement::get_followers_star, &Statement::get_followees_star);
                follows_parents_handler->set_args(pkb_, synonym_to_entity_result_, relationship, entities_to_return_);
                follows_parents_handler->Evaluate();
                break;
            }
            ...
            case RelRef::UsesS: {
                ...
                uses_modifies_handler->set_function_pointers(&Statement::get_uses, &Variable::get_stmts_using);
                uses_modifies_handler->set_args(pkb_, synonym_to_entity_result_, relationship, entities_to_return_);
            ...
            case RelRef::ModifiesS: {
              UsesModifiesHandler *uses_modifies_handler =
                  UsesModifiesHandler::get_instance();
              uses_modifies_handler->set_function_pointers(&Statement::get_modifies, &Variable::get_stmts_modifying);
              uses_modifies_handler->set_args(pkb_, synonym_to_entity_result_, relationship, entities_to_return_);
              uses_modifies_handler->Evaluate();
              break;
            }
            case RelRef::UsesP: {
              ...
              usesp_modifiesp_handler->set_function_pointers(&Procedure::get_uses, &Variable::get_procs_using);
              usesp_modifiesp_handler->set_args(pkb_, synonym_to_entity_result_, relationship, entities_to_return_);
            ...
            case RelRef::ModifiesP: {
              UsesPModifiesPHandler *usesp_modifiesp_handler =
                  UsesPModifiesPHandler::get_instance();
              usesp_modifiesp_handler->set_function_pointers(&Procedure::get_modifies, &Variable::get_procs_modifying);
              usesp_modifiesp_handler->set_args(pkb_, synonym_to_entity_result_, relationship, entities_to_return_);
              usesp_modifiesp_handler->Evaluate();
              break;
            }
