...
ExprOp ExprOpFromToken(const Token *t) {
  switch (t->kind_) {
    case TokenType::Plus:return ExprOp::Plus;
    case TokenType::Minus:return ExprOp::Minus;
    case TokenType::Multiply:return ExprOp::Times;
    case TokenType::Divide:return ExprOp::Divide;
    case TokenType::Modulo:return ExprOp::Modulo;
    default:
...
std::string ExprOpToString(ExprOp op) {
  switch (op) {
    case ExprOp::Plus:return "+";
    case ExprOp::Minus:return "-";
    case ExprOp::Times:return "*";
    case ExprOp::Divide:return "/";
    case ExprOp::Modulo:return "%";
    default:throw std::invalid_argument("ExprOpToString: invalid ExprOp value");
  }
}
  ...
  switch (op1->kind_) {
    ...
    case TokenType::Minus:
      switch (op2->kind_) {
        ...
        case TokenType::Multiply:
        case TokenType::Divide:
        case TokenType::Modulo:return false;

        default:throw std::invalid_argument("Precedes: invalid operator token for op2");
      }

    default:throw std::invalid_argument("Precedes: invalid operator token for op1");
  }
}
...
Node *ParseExpression(BufferedLexer *lexer, ParseState *state) {
  ...
  while (IsExpressionToken(next_token)) {
    switch (next_token->kind_) {
      case TokenType::Constant:result_stack.push(MakeConstantNodeFromToken(next_token));
        expr_str_stack.push(next_token->value_);
        break;

      case TokenType::Identifier:result_stack.push(MakeIdentifierNodeFromToken(next_token));
        expr_str_stack.push(next_token->value_);
        break;

      case TokenType::LParen:operator_stack.push(next_token);
        break;

      case TokenType::RParen:ProcessRightParen(next_token, &result_stack, &expr_str_stack, &operator_stack);
        break;
