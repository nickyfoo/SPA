...
void PKB::CallsProcessCallNode(Node *node, std::vector<Node *> &ancestorList) {
  ...
  for (Node *n : ancestorList) {
    if (n->get_kind() == NodeType::Procedure) {
      ...
      auto *pkb_calling_proc = proc_table_.get_procedure(caller_proc_name);
      auto *pkb_called_proc = proc_table_.get_procedure(called_proc_name);
...
std::set<int> PKB::GetLastStmts(StatementNode *node) {
  std::set<int> ans;
...
void PKB::LastStmtsOfProcedure(int u, std::set<int> &visited, std::set<int> &ans) {
  ...
  if (stmt_table_.get_statement(u)->get_kind() == NodeType::While
      && cfg_al_.find(u) != cfg_al_.end()
      && cfg_al_[u].size() == 1) {
    ans.insert(u);
    return;
  }
...
void PKB::CFGProcessProcedureNode(Node *node) {
  ...
  for (auto &stmt : stmt_lst) {
    if (stmt->get_kind() == NodeType::Call) {
      ...
      int first_stmt_no = proc_table_.get_procedure(proc_name)->get_stmt_no();

      // Add edge from call node to start of procedure
      cfg_bip_al_[call_stmt_no].insert({first_stmt_no, call_stmt_no});
      reverse_cfg_bip_al_[first_stmt_no].insert({call_stmt_no, -first_stmt_no});
    }
  }
}
...
void PKB::CFGProcessIfNode(Node *node) {
  ...
  if (!then_stmt_lst.empty()) {
    ...
    cfg_al_[if_stmt_no].insert(then_first_stmt_no);
    reverse_cfg_al_[then_first_stmt_no].insert(if_stmt_no);
    cfg_bip_al_[if_stmt_no].insert({then_first_stmt_no, kNoBranch});
    reverse_cfg_bip_al_[then_first_stmt_no].insert({if_stmt_no, kNoBranch});
  }
  ...
  for (int i = 1; i < then_stmt_lst.size(); i++) {
    ...
    for (auto &last_stmt : GetLastStmts(then_stmt_lst[i - 1])) {
      cfg_al_[last_stmt].insert(stmt_no);
      reverse_cfg_al_[stmt_no].insert(last_stmt);

      if (then_stmt_lst[i - 1]->get_kind() != NodeType::Call) {
        cfg_bip_al_[last_stmt].insert({stmt_no, kNoBranch});
        reverse_cfg_bip_al_[stmt_no].insert({last_stmt, kNoBranch});
      }
    }
  }
  ...
  if (!else_stmt_lst.empty()) {
    ...
    cfg_al_[if_stmt_no].insert(else_first_stmt_no);
    reverse_cfg_al_[else_first_stmt_no].insert(if_stmt_no);
    cfg_bip_al_[if_stmt_no].insert({else_first_stmt_no, kNoBranch});
    reverse_cfg_bip_al_[else_first_stmt_no].insert({if_stmt_no, kNoBranch});
  }
...
void PKB::CFGProcessWhileNode(Node *node) {
  ...
  if (!stmt_lst.empty()) {
    ...
    cfg_al_[while_stmt_no].insert(first_stmt_no);
    reverse_cfg_al_[first_stmt_no].insert(while_stmt_no);
    cfg_bip_al_[while_stmt_no].insert({first_stmt_no, kNoBranch});
    reverse_cfg_bip_al_[first_stmt_no].insert({while_stmt_no, kNoBranch});
  }
  ...
  if (!stmt_lst.empty()) {
    for (auto &last_stmt : GetLastStmts(stmt_lst[stmt_lst.size() - 1])) {
      cfg_al_[last_stmt].insert(while_stmt_no);
      reverse_cfg_al_[while_stmt_no].insert(last_stmt);

      if (stmt_table_.get_statement(last_stmt)->get_kind() != NodeType::Call) {
        // TODO(Nicholas): Dangerous, might mess something up
        cfg_bip_al_[last_stmt].insert({while_stmt_no, kNoBranch});
        reverse_cfg_bip_al_[while_stmt_no].insert({last_stmt, kNoBranch});
      }
    }
  }
...
void PKB::LinkProcedures() {
  for (auto &call_stmt : stmt_table_.get_statements(NodeType::Call)) {
    ...
    int first_stmt_of_proc = proc_table_.get_procedure(called_proc_name)->get_stmt_no();
    ...
    for (auto &last_stmt : ans) {
      for (auto &next_stmt : cfg_al_[call_stmt_no]) {
        cfg_bip_al_[last_stmt].insert({next_stmt, -call_stmt_no});
        reverse_cfg_bip_al_[next_stmt].insert({last_stmt, call_stmt_no});
      }
    }
  }
}
...
void PKB::ProcessCallStacks(std::set<std::pair<int, std::string>> &visited,
