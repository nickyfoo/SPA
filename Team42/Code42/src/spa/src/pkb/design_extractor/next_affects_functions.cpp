...
std::set<std::pair<int, int>> *PKB::get_affects(int a, int b) {
  ...
  if (a == kWild && b == kWild) {
    for (auto &stmt : stmt_table_.get_statements(NodeType::Assign)) {
      ...
      bool found = false;
      AffectsDFS(stmt->get_stmt_no(), kWild, stmt->get_stmt_no(),
                 var_name, visited, d, found);
    }
    ...
    for (auto &var_used : *(stmt_b->get_uses())) {
      for (auto &stmt_using : *(var_table_.get_variable(var_used)->get_stmts_modifying())) {
        if (stmt_checked[stmt_using]) continue;
    ...
    AffectsDFS(stmt_a->get_stmt_no(), kWild, stmt_a->get_stmt_no(), var_name, visited, d, found);
    ...
    bool found = false;
    std::string var_name = *(stmt_a->get_modifies()->begin());
    AffectsDFS(stmt_a->get_stmt_no(), b, stmt_a->get_stmt_no(), var_name, visited, d, found);
...
void PKB::AffectsDFS(int start, int target, int u, const std::string &var_name,
