...
TEST_CASE("Test 1: Correctly adding entities to entitiesMap") {
    EntityDeclarationParser* entityParser = EntityDeclarationParser::get_instance();
    ...
    unordered_map<string, EntityDeclaration*> *entitiesMap = entityParser->get_entities_map();
    REQUIRE(expectedEntitiesMap.size() == entitiesMap->size());
    REQUIRE(expectedEntitiesMap.at("s1")->get_synonym() == entitiesMap->at("s1")->get_synonym());
    REQUIRE(entitiesMap->at("s1")->get_type() == EntityType::Stmt);
    REQUIRE(expectedEntitiesMap.at("s2")->get_synonym() == entitiesMap->at("s2")->get_synonym());
    REQUIRE(entitiesMap->at("s2")->get_type() == EntityType::Stmt);
}

TEST_CASE("Test 2: Correctly adding entities to entitiesMap") {
    EntityDeclarationParser* entityParser = EntityDeclarationParser::get_instance();
    vector<string>* entities = new vector<string>({"assign assign", "procedure procedure"});
    entityParser->set_entities(entities);

    unordered_map<string, EntityDeclaration*> expectedEntitiesMap = {
            {"assign", new EntityDeclaration(EntityType::Assign, "assign")},
            {"procedure", new EntityDeclaration(EntityType::Procedure, "procedure")}
    };
    unordered_map<string, EntityDeclaration*>* entitiesMap = entityParser->get_entities_map();
    REQUIRE(expectedEntitiesMap.size() == entitiesMap->size());
    REQUIRE(expectedEntitiesMap.at("assign")->get_synonym() == entitiesMap->at("assign")->get_synonym());
    REQUIRE(entitiesMap->at("assign")->get_type() == EntityType::Assign);
    REQUIRE(expectedEntitiesMap.at("procedure")->get_synonym() == entitiesMap->at("procedure")->get_synonym());
...
TEST_CASE("Test 3: Having multiple synonyms for 1 entity type_") {
    EntityDeclarationParser* entityParser = EntityDeclarationParser::get_instance();
    vector<string>* entities = new vector<string>({"stmt s, s2", "procedure p"});
    entityParser->set_entities(entities);
    ...
    unordered_map<string, EntityDeclaration*>* entitiesMap = entityParser->get_entities_map();
    REQUIRE(expectedEntitiesMap.size() == entitiesMap->size());
    REQUIRE(expectedEntitiesMap.at("s")->get_synonym() == entitiesMap->at("s")->get_synonym());
    REQUIRE(entitiesMap->at("s")->get_type() == EntityType::Stmt);
    REQUIRE(expectedEntitiesMap.at("p")->get_synonym() == entitiesMap->at("p")->get_synonym());
...
TEST_CASE("Invalid test: repeat synonym name") {
    EntityDeclarationParser* entityParser = EntityDeclarationParser::get_instance();
    vector<string>* entities = new vector<string>({"stmt s1, s1", "procedure s"});
    entityParser->set_entities(entities);
    unordered_map<string, EntityDeclaration*>* entitiesMap = entityParser->get_entities_map();
